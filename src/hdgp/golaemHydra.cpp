...
#undef CMP
        ...
        struct MeshKey
        {
            ...
            {
                return characterIndex == other.characterIndex && lodIndex == other.lodIndex && meshIndex == other.meshIndex;
            }

            std::size_t hash() const noexcept
            {
                constexpr int bits = sizeof(std::size_t) * 8 / 3; // 10 or 21
                return static_cast<std::size_t>(lodIndex) << (2 * bits) | static_cast<std::size_t>(characterIndex) << bits | static_cast<std::size_t>(meshIndex);
            }
        ...
        private:
            ...
            void GenerateFBXMeshes(
                MeshEntityData& meshEntityData, const glm::Array<HdSampledDataSource::Time>& shutterOffsets,
                const glm::crowdio::InputEntityGeoData& inputData,
                const glm::crowdio::OutputEntityGeoData& outputData,
                const PrimvarDSMapRef& customPrimvars);
            ...
            static const HdContainerDataSourceHandle primvarsDs =
                HdRetainedContainerDataSource::New(
                    ...
                    HdPrimvarSchema::Builder()
                        ...
                        .SetInterpolation(glm::hydra::GetVertexInterpDataSource())
        ...
        void GolaemProcedural::PopulateCrowd(const HdSceneIndexBaseRefPtr& inputScene)
        {
            ...
            for (size_t ifield = 0; ifield < _args.crowdFields.size(); ++ifield)
            {
            ...
            const GolaemCharacter* character, int entityIndex) const
        {
            ...
            size_t totalCount = shaderAttrCount + simData->_ppFloatAttributeCount + simData->_ppVectorAttributeCount;
            ...
            const GfVec3d& cameraPos, const GfVec3d& entityPos, size_t* lodLevel)
        {
            ...
            glm::Array<HdSampledDataSource::Time> shutterOffsets;
            ...
            const PrimvarDSMapRef& customPrimvars)
        {
            ...
            for (size_t imesh = 0; imesh < meshCount; ++imesh)
            {
                ...
                bool isRigid = glm::hydra::kEnableRigidEntities && fileMesh._skinningType == glm::crowdio::GLM_SKIN_RIGID;
            ...
            const PrimvarDSMapRef& customPrimvars)
        {
            ...
            if (behavior._idGeometryFileIdx >= 0)
            {
                ...
                for (size_t isample = 0; isample < shutterOffsets.size(); ++isample)
                {
                    const float (&geoCacheData)[3] =
                        inputData._frameDatas[isample]->_geoBehaviorAnimFrameInfo[behavior._geoDataIndex];
        ...
        GolaemProcedural::UpdateDependencies(const HdSceneIndexBaseRefPtr& inputScene)
        {
            ...
            if (_args.enableMotionBlur)
            {
                ...
                if (globals)
                {
                    HdPathDataSourceHandle rsPrimDS = globals.GetActiveRenderSettingsPrim();
                    bool hasActiveRenderSettingsPrim = rsPrimDS != nullptr;
                    if (hasActiveRenderSettingsPrim)
                    {
                        ...
                        {
                            hasActiveRenderSettingsPrim = false;
                        }
                    }
                    if (!hasActiveRenderSettingsPrim && !camPath.IsEmpty())
                    {
                        TF_DEBUG_MSG(
                            GLMHYDRA_DEPENDENCIES,
        ...
        HdGpGenerativeProcedural::ChildPrimTypeMap GolaemProcedural::Update(
            ...
            HdSceneIndexObserver::DirtiedPrimEntries* outputDirtiedPrims)
        {
            if (TfDebug::IsEnabled(GLMHYDRA_DEPENDENCIES) && dirtiedDependencies.size() > 0)
            {
            ...
            if (_args.displayMode == golaemTokens->bbox)
            {
                ...
                for (size_t i = 0; i < _bboxEntities.size(); ++i)
                {
                    ...
                    if (previousResult.find(childPath) != previousResult.end())
                    {
            ...
            else
            {
                ...
                for (size_t i = 0; i < _meshEntities.size(); ++i)
                {
                    ...
                    result[groupPath] = TfToken();
                    _childIndexPairs[groupPath] =
                        {i, std::numeric_limits<size_t>::max()};

                    if (previousResult.find(groupPath) != previousResult.end())
                    {
                        outputDirtiedPrims->emplace_back(
                            groupPath, updateAll
